...
const BuyKasuLock = () => {
  ...
  const { selectedLockPeriod, setSelectedLockPeriod, swapAndLock } =
  ...
  useEffect(() => {
    if (!selectedLockPeriod && lockPeriods?.length > 0) {
      setSelectedLockPeriod(lockPeriods[0])
    }
  }, [selectedLockPeriod, lockPeriods, setSelectedLockPeriod])
        ...
        <Box display='flex' flexDirection='column' alignItems='center'>
          <Slider
            ...
            valueLabelFormat={(val) =>
              `${parseFloat(lockPeriods[val].lockPeriod)} days`
            }
            color='secondary'
