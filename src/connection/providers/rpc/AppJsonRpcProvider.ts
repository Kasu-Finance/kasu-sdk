...
export default class AppRpcProvider extends AppStaticJsonRpcProvider {
  ...
  constructor(
    ...
    providers.forEach((provider, i) => {
      if (!Provider.isProvider(provider))
        throw new Error(`invalid provider ${i}`)
    })
  ...
  async perform(method: string, params: { [name: string]: any }): Promise<any> {
    ...
    this.providerEvaluations.forEach((providerEval) => {
      if (
        currentTime - providerEval.performance.lastEvaluated >=
    ...
    this.providerEvaluations = AppRpcProvider.sortProviders(
      this.providerEvaluations.slice()
    )
  ...
  static sortProviders(providerEvaluations: FallbackProviderEvaluation[]) {
    return providerEvaluations.sort((a, b) => {
      ...
      const aAverageLatency =
        a.performance.latency / (a.performance.callCount || 1)
      const aFailRate =
        (a.performance.failureCount || 0.01) / (a.performance.callCount || 1)

      // Provider b calculations
      const bAverageLatency =
        b.performance.latency / (b.performance.callCount || 1)
      const bFailRate =
        (b.performance.failureCount || 0.01) / (b.performance.callCount || 1)
