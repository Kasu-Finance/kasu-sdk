...
import { ISystemVariablesAbi, ISystemVariablesAbi__factory } from '../../contracts';
...
export class Portfolio {
    ...
    async getPortfolioSummary(userAddress: string): Promise<PortfolioSummary> {
        ...
        const lastEpochData: LastEpochQueryResult = await this._graph.request(lastEpochQuery, {userAddress: userAddress, epochId: latestEpoch.toString()});
        ...
        for (const lastEpochDatapoint of lastEpochData.user.lendingPoolUserDetails) {
            if(lastEpochDatapoint.lendingPoolTrancheUserDetails.length == 0) continue;
            const prevTrancheYield = lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheEpochInterest.length > 0 ? parseFloat(lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheEpochInterest[0].epochInterestAmount) : 0;
            ...
            totalYieldEarnedLastEpoch += prevTrancheShares != 0 ? (prevUserShares * prevTrancheYield) / prevTrancheShares : 0;
        }
        return {
    ...
    async getPortfolioLendingData(userAddress: string): Promise<LendingPortfolioData> {
        ...
        for(const poolOverview of poolOverviews) {
            ...
            for (const tranche of poolOverview.tranches) {
                const lastEpochDatapoint = lastEpochData.user.lendingPoolUserDetails.find(l => l.lendingPoolTrancheUserDetails[0].tranche.id == tranche.id);
                if(!lastEpochDatapoint || lastEpochDatapoint.lendingPoolTrancheUserDetails.length == 0) continue;
                const prevTrancheYield = lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheEpochInterest.length > 0 ? parseFloat(lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheEpochInterest[0].epochInterestAmount) : 0;
                const prevUserShares = lastEpochDatapoint.lendingPoolTrancheUserDetails[0].lendingPoolTrancheUserEpochSharesUpdates.length > 0 ? parseFloat(lastEpochDatapoint.lendingPoolTrancheUserDetails[0].lendingPoolTrancheUserEpochSharesUpdates[0].shares) : 0;
                const prevTrancheShares = lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheShareUpdates.length > 0 ? parseFloat(lastEpochDatapoint.lendingPoolTrancheUserDetails[0].tranche.lendingPoolTrancheShareUpdates[0].shares) : 0;
